				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<div class="btn-group m-r">
							<select id="state" name="roleFormMap.state"
								class="form-control m-b">
								<option value="0"
									<c:if test="${role.state eq 0}">selected="selected"</c:if>>是</option>
								<option value="1"
									<c:if test="${role.state eq 1}">selected="selected"</c:if>>否</option>
							</select>

							<%-- <button data-toggle="dropdown"
								class="btn btn-sm btn-default dropdown-toggle">
								<span class="dropdown-label"><c:if
										test="${role.state eq 0}">是</c:if> <c:if
										test="${role.state eq 1}">否</c:if></span> <span class="caret"></span>
							</button>
							<ul class="dropdown-menu dropdown-select">
								<li class=""><a href="#"><input type="radio"
										name="roleFormMap.state" value="0"
										<c:if test="${role.state eq 0}"> checked="checked"</c:if>>是</a></li>
								<li class="active"><a href="#"><input type="radio"
										name="roleFormMap.state" value="1"
										<c:if test="${role.state eq 1}"> checked="checked"</c:if>>否</a></li>
							</ul> --%>
						</div>
					</div>
				</div>
